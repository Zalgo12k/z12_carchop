		...
		if not IsEntityAttachedToEntity(partID, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					Packaging = false
						if door1gone then
							exports['progressBars']:startUI(5000, "Parçayı arıyorsun.")
							Citizen.Wait(5000)
							TriggerServerEvent("z12:reward:kapi")
							TriggerServerEvent("z12:reward:jant")
							TriggerServerEvent("z12:server:reward")
							door1gone = false
							door1delivered = true
		...
		if not IsEntityAttachedToEntity(partID, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					Packaging = false
						if door5gone then
							TriggerServerEvent("z12:reward:kaput")
							exports['progressBars']:startUI(5000, "Parçayı arıyorsun.")
							Citizen.Wait(5000)
							TriggerServerEvent("z12:server:reward")
							door5gone = false
						...
						if door6gone then
							TriggerServerEvent("z12:reward:bagaj")
							exports['progressBars']:startUI(5000, "Parçayı arıyorsun.")
							Citizen.Wait(5000)
							TriggerServerEvent("z12:server:reward")
							door6gone = false
						...
						if door7gone then
							TriggerServerEvent("z12:reward:radyo")
							exports['progressBars']:startUI(5000, "Parçayı arıyorsun.")
							Citizen.Wait(5000)
							TriggerServerEvent("z12:server:reward")
							door7gone = false
...
function FinishedChopping()
	...
	crossing = GetStreetNameFromHashKey(crossing)
				...
				print('Polise 1. Bildirim Gitti.')
				TriggerServerEvent('m3:dispatch:notify', 'Araç Parçalama', 'Anonim', 'Yok', pCoords)

TriggerServerEvent('esx_civlife_illegalchop:success', math.random(10000,20000))
TriggerServerEvent("z12:server:reward")
TriggerServerEvent("z12:server:reward")
TriggerServerEvent("z12:server:reward")
TriggerServerEvent("z12:server:reward")
